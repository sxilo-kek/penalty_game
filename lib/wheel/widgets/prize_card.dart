...
class PrizeCard extends StatelessWidget {
  const PrizeCard({super.key, required this.prize, this.width, this.height});
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Container(
          ...
          constraints: BoxConstraints(maxWidth: w.isFinite ? w : constraints.maxWidth, maxHeight: h.isFinite ? h : constraints.maxHeight),
          decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(WheelLayout.cardBorderRadius),
            boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.16), blurRadius: 36, offset: const Offset(0, 18))],
          ),
          padding: const EdgeInsets.all(WheelLayout.cardPadding),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Expanded(child: Image.asset('${AssetPaths.wheelImages}${prize.image}', fit: BoxFit.contain)),
            ],
          ),
        );
      },
